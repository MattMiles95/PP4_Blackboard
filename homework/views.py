...
class HomeworkSubmissionView(LoginRequiredMixin, View):
    ...
    **GET Method**
    Displays a form for submitting homework, filtered by the specified
    subject.
    ...
    **Template:**
    ...
    def post(self, request, subject):
        ...
        if form.is_valid():
            ...
            homework.save()
            messages.add_message(request, messages.SUCCESS, 'Homework '
                                 'submitted. Good job!')
            return redirect('homework_dashboard')
        else:
            messages.add_message(request, messages.ERROR, 'Hmm, '
